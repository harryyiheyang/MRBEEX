MRBEE_Mixture_SuSiE=function(by,bX,byse,bXse,LD,Rxy,cluster.index=c(1:length(by)),main.cluster.thres=0.45,min.cluster.size=5,Lvec=c(1:min(5,ncol(bX))),pip.thres=0.2,ebic.theta=1,reliability.thres=0.6,sampling.time=100,max.iter=30,max.eps=5e-4,sampling.iter=5,susie.iter=100,ridge.diff=1e5,projection.eigen.floor=1,verbose=T,pip.min=0.1,cred.pip.thres=0.95,estimate_residual_variance=T,group.penalize=F,group.index=c(1:ncol(bX)[1]),group.diff=10,coverage.causal=0.95,estimate_residual_method="MoM",standardize=T,tau=5,step.size=0.5,theta.ini.1=NULL,theta.ini.2=NULL){
  t1=Sys.time()
  by=by/byse
  byseinv=1/byse
  bX=bX*byseinv
  ...
  for(v in 1:length(Lvec)){
    ...
    for(l in 1:v){
      ...
      while(iter<max.iter&error>max.eps){
        ...
        fit.susie1=tryCatch({
          susie_ss(XtX=XtX1,Xty=Xty1,yty=yty1,n=length(cluster1),L=Lvec[v],max_iter=susie.iter,estimate_prior_method=ifelse(is.null(fit.susie1),"optim","EM"),model_init=fit.susie1,coverage = coverage.causal,estimate_residual_variance=estimate_residual_variance,residual_variance=max(0.9,vary),estimate_residual_method=estimate_residual_method,standardize=standardize)
        },error = function(e) {
          susie_ss(XtX=XtX1,Xty=Xty1,yty=yty1,n=length(cluster1),L=Lvec[v],max_iter=susie.iter,estimate_prior_method=ifelse(is.null(fit.susie1),"optim","EM"),model_init=fit.susie1,estimate_residual_variance=F,residual_variance=max(0.9,vary),coverage = coverage.causal,estimate_residual_method=estimate_residual_method,standardize=standardize)
        })
        ...
        if(length(cluster2)>min.cluster.size){
          ...
          fit.susie2=tryCatch({
            susie_ss(XtX=XtX2,Xty=Xty2,yty=yty2,n=length(cluster2),L=Lvec[l],max_iter=susie.iter,estimate_prior_method=ifelse(is.null(fit.susie2),"optim","EM"),model_init=fit.susie2,coverage = coverage.causal,estimate_residual_variance=estimate_residual_variance,residual_variance=max(0.9,vary),estimate_residual_method=estimate_residual_method,standardize=standardize)
          },error = function(e) {
            susie_ss(XtX=XtX2,Xty=Xty2,yty=yty2,n=length(cluster2),L=Lvec[l],max_iter=susie.iter,estimate_prior_method=ifelse(is.null(fit.susie2),"optim","EM"),model_init=fit.susie2,estimate_residual_variance=F,residual_variance=max(0.9,vary),coverage = coverage.causal,estimate_residual_method=estimate_residual_method,standardize=standardize)
          })
  ...
  while(iter<(max.iter*2)&error>max.eps){
    ...
    XtX1=project_XtX1(XtX1+Cmat1+Diff_matrix1, Cmat1, cluster1, projection.eigen.floor1)
    fit.susie1=tryCatch({
      susie_ss(XtX=XtX1,Xty=Xty1,yty=yty1,n=length(cluster1),L=Lvec[vstar],max_iter=susie.iter,estimate_prior_method=ifelse(is.null(fit.susie1),"optim","EM"),model_init=fit.susie1,coverage = coverage.causal,estimate_residual_variance=estimate_residual_variance,residual_variance=max(0.9,vary),estimate_residual_method=estimate_residual_method,standardize=standardize)
    },error = function(e) {
      susie_ss(XtX=XtX1,Xty=Xty1,yty=yty1,n=length(cluster1),L=Lvec[vstar],max_iter=susie.iter,estimate_prior_method=ifelse(is.null(fit.susie1),"optim","EM"),model_init=fit.susie1,estimate_residual_variance=F,residual_variance=max(0.9,vary),coverage = coverage.causal,estimate_residual_method=estimate_residual_method,standardize=standardize)
    })
    theta1=coef.susie(fit.susie1)[-1]*(fit.susie1$pip>pip.min)
    ...
    if(length(cluster2)>min.cluster.size){
      ...
      XtX2=project_XtX2(XtX2+Cmat2+Diff_matrix2, Cmat2, cluster2, projection.eigen.floor2)
      fit.susie2=tryCatch({
        susie_ss(XtX=XtX2,Xty=Xty2,yty=yty2,n=length(cluster2),L=Lvec[lstar],max_iter=susie.iter,estimate_prior_method=ifelse(is.null(fit.susie2),"optim","EM"),model_init=fit.susie2,coverage = coverage.causal,estimate_residual_variance=estimate_residual_variance,residual_variance=max(0.9,vary),estimate_residual_method=estimate_residual_method,standardize=standardize)
      },error = function(e) {
        susie_ss(XtX=XtX2,Xty=Xty2,yty=yty2,n=length(cluster2),L=Lvec[lstar],max_iter=susie.iter,estimate_prior_method=ifelse(is.null(fit.susie2),"optim","EM"),model_init=fit.susie2,estimate_residual_variance=F,residual_variance=max(0.9,vary),coverage = coverage.causal,estimate_residual_method=estimate_residual_method,standardize=standardize)
      })
      theta2=coef.susie(fit.susie2)[-1]*(fit.susie2$pip>pip.min)
