MRBEE_Mixture_SuSiE=function(by,bX,byse,bXse,LD,Rxy,cluster.index=c(1:length(by)),main.cluster.thres=0.48,min.cluster.size=5,Lvec=c(1:min(5,ncol(bX))),pip.thres=0.2,ebic.theta=1,reliability.thres=0.8,sampling.time=100,robust.se=T,max.iter=30,max.eps=5e-4,sampling.iter=5){
########################### Basic information #######################
by=by/byse
byseinv=1/byse
bX=bX*byseinv
...
Bbic[v]=MRFit(by=tilde.y,bX=tilde.X,theta1=theta1,theta2=theta2,cluster1=cluster1,cluster2=cluster2,df1=df1,df2=df2)+(df2+df1)*(log(p*2)*ebic.theta+log(m))
}
Bbic=Bbic/m
######################## Final Estimate #################################
...
cluster.ratio=c(length(cluster1j),length(cluster2j))/m
}
