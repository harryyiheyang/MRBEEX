MRBEE_Mixture=function(by,bX,byse,bXse,LD,Rxy,cluster.index=c(1:length(by)),main.cluster.thres=0.48,min.cluster.size=5,reliability.thres=0.8,sampling.time=100,robust.se=T,ebic.theta=1,max.iter=30,max.eps=5e-4,sampling.iter=5){
########################### Basic information #######################
by=by/byse
byseinv=1/byse
bX=bX*byseinv
...
Bic=MRFit(by=tilde.y,bX=tilde.X,theta1=theta1,theta2=theta2,cluster1=cluster1,cluster2=cluster2,df1=p,df2=p)+2*p*(log(p*2)*ebic.theta+log(m))
Bic=Bic/m
}
