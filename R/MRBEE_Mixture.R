MRBEE_Mixture=function(by,bX,byse,bXse,LD,Rxy,cluster.index=c(1:length(by)),main.cluster.thres=0.45,min.cluster.size=5,reliability.thres=0.6,sampling.time=100,ebic.theta=1,max.iter=30,max.eps=5e-4,sampling.iter=5,verbose=T,group.penalize=F,group.index=c(1:ncol(bX)[1]),group.diff=10,tau=5,step.size=0.5){
########################### Basic information #######################
t1=Sys.time()
by=by/byse
byseinv=1/byse
...
sigma1=sum((tilde.y[cluster1]-tilde.X[cluster1,,drop=FALSE]%*%theta1)^2)/(length(cluster1)-sum(theta1!=0))
sigma2=sum((tilde.y[cluster2]-tilde.X[cluster2,,drop=FALSE]%*%theta2)^2)/(length(cluster2)-sum(theta2!=0))
...
gamma = soft(ifelse(abs(initial_res) > quantile(abs(initial_res), 0.975), initial_res, 0),1) # Shrinkage trick
t2=Sys.time()
...
sigma1=sum((tilde.res[cluster1]-tilde.X[cluster1,,drop=FALSE]%*%theta1)^2)/(length(cluster1)-p)
sigma2=sum((tilde.res[cluster2]-tilde.X[cluster2,,drop=FALSE]%*%theta2)^2)/(length(cluster2)-p)
sigma2=max(0.25,sigma2)
sigma1=max(0.25,sigma1)
...
Bic=MRFit(by=tilde.res,bX=tilde.X,theta1=theta1,theta2=theta2,cluster1=cluster1,cluster2=cluster2,df1=p,df2=p)+2*p*(log(p*2)*ebic.theta+log(m))+log(m)
Bic=Bic/m
}
t2=Sys.time()
time_to_print=round(difftime(t2, t1, units = "secs"),3)
