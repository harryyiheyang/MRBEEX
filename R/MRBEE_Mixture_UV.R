MRBEE_Mixture_UV=function(by,bX,byse,bXse,Rxy,LD=LD,main.cluster.thres=0.4,cluster.index=cluster.index,reliability.thres=0.8,sampling.time=100,min.cluster.size=5,robust.se=T,ebic.theta=1,max.iter=30,max.eps=5e-4,sampling.iter=5){
########################### Basic information #######################
sink(tempfile())
by=by/byse
byseinv=1/byse
...
Voting=cluster_voting_uv(by=by,bX=bX,cluster.index=cluster.index,theta1=theta1,theta2=theta2,sigma1=sigma1,sigma2=sigma2,main.cluster.thres=main.cluster.thres)
...
if(length(cluster2j)>min.cluster.size){
...
theta2j=0*theta1j
cluster2j=sample(length(indj),min.cluster.size)
}
...
Votingj=cluster_voting_uv(by=byj,bX=bXj,cluster.index=cluster.index[indj],theta1=theta1j,theta2=theta2j,sigma1=sigma1j,sigma2=sigma2j,main.cluster.thres=main.cluster.thres)
