MRBEE_Mixture_UV=function(by,bX,byse,bXse,Rxy,LD=LD,cluster.index=cluster.index,reliability.thres=0.8,sampling.time=100,ebic.theta=1,max.iter=30,max.eps=5e-4,sampling.iter=5,LDSC=NULL,Omega=NULL){
########################### Basic information #######################
by=by/byse
byseinv=1/byse
bX=bX*byseinv
...
############################### inference #########################
names(theta1)=names(theta2)=names(bX)
...
errorj <- abs(theta1j - theta_prev1j)
if(errorj<max.eps&iterj>3) break
}
