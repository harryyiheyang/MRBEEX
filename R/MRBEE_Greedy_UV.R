MRBEE_Greedy_UV=function(by,bX,byse,bXse,LD=LD,Rxy,cluster.index,Kvec=seq(0,floor(0.5*length(by)),by=2),max.iter=100,max.eps=0.001,ebic.gamma=1,maxdiff=1.5,sampling.time=100,sampling.iter=5,theta.ini=F,gamma.ini=F,reliability.thres=0.6){
########################### Basic information #######################
by=by/byse
byseinv=1/byse
bX=bX*byseinv
...
Hinvj=1/(BtB-sum(bXse[indvalidj]^2)*Rxy[1,1])
g=sum(Bt*(by[indj]-as.vector(LD[indj,indj]%*%gammaj)))-sum(bXse[indvalidj])*Rxy[2,1]
thetaj=g*Hinvj
if(Kvec[jstar]>0){
