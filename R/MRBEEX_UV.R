...
MRBEEX_UV=function(by,bX,byse,bXse,LD="identity",Rxy,cluster.index=c(1:length(by)),
        ...
        tauvec=seq(5,30,by=2),rho=2,ebic.gamma=2,
        max.iter=100,max.eps=0.001,maxdiff=3,
        sampling.time=1000,sampling.iter=30,
...
##########################################################################
if(Method[1]=="Mixture"){
A=MRBEE_Mixture_UV(by=by,bX=bX,byse=byse,bXse=bXse,LD=LD,Rxy=Rxy,cluster.index=cluster.index,reliability.thres=reliability.thres,sampling.time=sampling.time,ebic.theta=ebic.theta,max.iter=max.iter,max.eps=max.eps,sampling.iter=sampling.iter,LDSC=ldsc,Omega=gcov)
}
