MRBEE_IPOD_UV=function(by,bX,byse,bXse,LD=LD,Rxy,cluster.index,tauvec=seq(3,50,by=2),max.iter=100,max.eps=0.001,ebic.gamma=1,rho=2,maxdiff=1.5,sampling.time=100,sampling.iter=5,theta.ini=F,gamma.ini=F,reliability.thres=0.6){
########################### Basic information #######################
by=by/byse
byseinv=1/byse
bX=bX*byseinv
...
g=sum(Bt*(by[indj]-as.vector(LD[indj,]%*%gammaj)))-sum(bXse[indvalidj])*Rxy[2,1]
thetaj=g*Hinv
...
BtB=sum(tilde.X^2)
h=(BtB-sum(bXse[indvalid]^2)*Rxy[1,1])
E=-tilde.X*e[indvalid]+bXse[indvalid]*byse[indvalid]-bXse[indvalid]^2*theta
...
Dmcp=c(0,ifelse(abs(gamma[indgamma])<3*tauvec[jstar],-1/3,0))
H=H+diag(Dmcp,ncol(H))
e=res
Hinv=solve(H)
E=-as.matrix(Theta%*%bZ)*e
for(i in 1:length(indvalid)){
E[indvalid[i],1]=E[indvalid[i],1]+bXse[indvalid[i]]*Rxy[1,2]-bXse[indvalid[i]]^2*Rxy[1,1]*theta
}
V=t(E)%*%E
