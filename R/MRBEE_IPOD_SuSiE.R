MRBEE_IPOD_SuSiE=function(by,bX,byse,bXse,LD,Rxy,cluster.index=c(1:length(by)),Lvec=c(1:min(10,nrow(bX))),pip.thres=0.5,tauvec=seq(3,50,by=2),max.iter=100,max.eps=0.001,susie.iter=100,ebic.theta=1,ebic.gamma=2,reliability.thres=0.8,rho=2,maxdiff=1.5,sampling.time=100,sampling.iter=10,theta.ini=F,gamma.ini=F,ridge.diff=1e5,verbose=T,pip.min=0.1,cred.pip.thres=0.95,group.penalize=F,group.index=c(1:ncol(bX)[1]),group.diff=10,coverage.causal=0.95,LDSC=NULL,Omega=NULL,estimate_residual_variance=T,prob.shrinkage=0.5){
########################### Basic information #######################
t1=Sys.time()
by=by/byse
byseinv=1/byse
...
tryCatch({
fit.theta=susie_suff_stat(XtX=XtX,Xty=Xty,yty=yty,n=m,L=Lvec[v],estimate_prior_method="EM",intercept=F,max_iter=susie.iter,s_init=fit.theta,coverage = coverage.causal,estimate_residual_variance=estimate_residual_variance,residual_variance=max(0.9,vary))
},error = function(e) {
fit.theta=susie_suff_stat(XtX=XtX,Xty=Xty,yty=yty,n=m,L=Lvec[v],estimate_prior_method="EM",intercept=F,estimate_residual_variance=F,residual_variance=max(0.9,vary),max_iter=susie.iter,s_init=fit.theta,coverage = coverage.causal)
})
...
yty=sum(res.theta*(Theta%*%res.theta))
tryCatch({
fit.theta=susie_suff_stat(XtX=XtX,Xty=Xty,yty=yty,n=m,L=Lvec[vstar],estimate_prior_method="EM",intercept=F,max_iter=susie.iter,s_init=fit.theta,coverage = coverage.causal,estimate_residual_variance=estimate_residual_variance,residual_variance=max(0.9,vary))
},error = function(e) {
fit.theta=susie_suff_stat(XtX=XtX,Xty=Xty,yty=yty,n=m,L=Lvec[vstar],estimate_prior_method="EM",intercept=F,estimate_residual_variance=F,residual_variance=max(0.9,vary),max_iter=susie.iter,s_init=fit.theta,coverage = coverage.causal)
})
theta=coef.susie(fit.theta)[-1]*(fit.theta$pip>pip.min)
theta.cs=group.pip.filter(pip.summary=summary(fit.theta)$var,xQTL.cred.thres=cred.pip.thres,xQTL.pip.thres=pip.thres)
pip.alive=theta.cs$ind.keep
...
tryCatch({
fit.thetaj=susie_suff_stat(XtX=XtXj,Xty=Xtyj,yty=ytyj,n=mj,L=Lvec[vstar],estimate_prior_method="EM",intercept=F,max_iter=sampling.iter,s_init=fit.theta,coverage = coverage.causal,estimate_residual_variance=estimate_residual_variance,residual_variance=max(0.9,vary))
},error = function(e) {
fit.thetaj=susie_suff_stat(XtX=XtXj,Xty=Xtyj,yty=ytyj,n=mj,L=Lvec[vstar],estimate_prior_method="EM",intercept=F,estimate_residual_variance=F,residual_variance=max(0.9,vary),max_iter=sampling.iter,s_init=fit.theta,coverage = coverage.causal)
})
