Cis_MRBEE_IPOD_SuSiE=function(by,bX,byse,bXse,LD,Rxy,Lvec=c(1:min(10,nrow(bX))),pip.thres=0.2,tauvec=seq(3,50,by=2),max.iter=100,max.eps=0.001,susie.iter=100,ebic.theta=1,ebic.gamma=2,reliability.thres=0.8,rho=2,maxdiff=1.5,theta.ini=F,gamma.ini=F,ridge=ridge,pleiotropy.rm=NULL){
########################### Basic information #######################
by=by/byse
byseinv=1/byse
bX=bX*byseinv
...
gamma.ini[is.na(gamma.ini)]=0
theta.ini[is.na(theta.ini)]=0
...
fit.theta=susie_suff_stat(XtX=XtX,Xty=Xty,yty=yty,n=m,L=Lvec[v],residual_variance=1,estimate_residual_variance=T,estimate_prior_method="EM",intercept=F,max_iter=susie.iter,standardize=F,s_init=fit.theta)
theta=coef.susie(fit.theta)[-1]*(fit.theta$pip>pip.thres)
theta.cs=group.pip.filter(pip.summary=summary(fit.theta)$var,pip.thres.cred=0.95)
...
fit.theta=susie_suff_stat(XtX=XtX,Xty=Xty,yty=yty,n=m,L=Lvec[vstar],residual_variance=1,estimate_prior_method="EM",intercept=F,estimate_residual_variance=T,max_iter=susie.iter*10,standardize=F,s_init=fit.theta)
theta=coef.susie(fit.theta)[-1]*(fit.theta$pip>pip.thres)
theta.cs=group.pip.filter(pip.summary=summary(fit.theta)$var,pip.thres.cred=0.95)
pip.alive=theta.cs$ind.keep
theta[-pip.alive]=0
