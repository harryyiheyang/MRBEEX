Cis_MRBEE_IPOD_SuSiE=function(by,bX,byse,bXse,LD,Rxy,Lvec=c(1:min(10,nrow(bX))),pip.thres=0.2,tauvec=seq(3,50,by=2),max.iter=100,max.eps=0.001,susie.iter=100,ebic.theta=1,ebic.gamma=2,reliability.thres=0.8,rho=2,theta.ini=F,gamma.ini=F,ridge=ridge,pleiotropy.rm=NULL){
########################### Basic information #######################
by=by/byse
byseinv=1/byse
bX=bX*byseinv
...
############################ Initial Estimate #######################
if(theta.ini[1]==F){
fit0=susie_suff_stat(XtX=BtB,Xty=c(t(bXinv)%*%by),yty=sum(by*(Theta%*%by)),n=m,intercept=F,estimate_prior_method="EM")
