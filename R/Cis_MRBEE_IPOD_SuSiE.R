...
H[1:length(indtheta),1:length(indtheta)]=H[1:length(indtheta),1:length(indtheta)]-Rxysum[indtheta,indtheta]+ridge*Diff[indtheta,indtheta]
Dmcp=c(rep(0,length(indtheta)),ifelse(abs(gamma[indgamma])<3*tauvec[jstar],-1/3,0))
H=as.matrix(H)+diag(Dmcp,ncol(H))
Hinv=solve(H)
