...
r=reliability.adj(bX,bXse*sqrt(Rxy[1,1]),Theta=Theta,thres=reliability.thres)
r=c(r,1)
Rxy=t(t(Rxy)*r)*r
...
fit.susie=tryCatch({
susie_ss(XtX=as.matrix(XtX),Xty=Xty,yty=yty,L=1,n=length(indvalid),estimate_prior_method=ifelse(is.null(fit.susie),"optim","EM"),residual_variance=1,model_init=fit.susie,max_iter=susie.iter,residual_variance_lowerbound=1)
},error = function(e) {
susie_ss(XtX=as.matrix(XtX),Xty=Xty,yty=yty,L=1,n=length(indvalid),estimate_prior_method=ifelse(is.null(fit.susie),"optim","EM"),residual_variance=1,model_init=fit.susie,max_iter=susie.iter,estimate_residual_variance=F)
})
