...
generate_D_matrix <- function(s, alpha = 0.5) {
## This function generate Diff2*alpha+Diff2*(1-alpha)
## Diff1 is the order-1 differential penalty
## Diff2 is the order-2 differential penalty
p <- length(s)
if (p == 1) {
D <- 0
} else if (p == 2) {
D <- matrix(c(1 / s[1], -1 / s[2]), nrow = 1, ncol = 2)
D=t(D)%*%D
} else {
...
D <- alpha*t(D1)%*%D1+(1-alpha)*t(D2)%*%D2
}
return(D)
}
