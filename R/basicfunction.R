utils::globalVariables(c(".","BP","CHR","P","SNP","center_BP","center_P",
  "cluster","cluster_id","count1","count2","dist_bp","i.BP","i.CHR",
  "i.P","i.SNP","idx","x.BP","x.P","x.SNP"))
...
q=dim(bZse)[2]
n=dim(bZse)[1]
S=array(rep(bZse,q),c(n,q,q))
RxyList=array(rep(Rxy,each=n),c(n,q,q))*S*aperm(S,c(1,3,2))
return(RxyList)
}
...
blocks <- split(seq_len(n),factor(cluster.index,levels=unique(cluster.index)))
block.size <- lengths(blocks)
selected <- integer(0)
remaining.units <- list()

single <- which(block.size < 2L)
if(length(single)>0L){
keep <- runif(length(single)) < 0.5
if(any(keep)) selected <- c(selected,unlist(blocks[single[keep]],use.names=FALSE))
if(any(!keep)) remaining.units <- c(remaining.units,blocks[single[!keep]])
}

pair <- which(block.size == 2L)
if(length(pair)>0L){
n.selected <- floor(length(pair)/2)
if(length(pair)%%2L==1L&&runif(1)<0.5) n.selected <- n.selected+1L
selected.blocks <- integer(0)
if(n.selected>0L){
selected.blocks <- sample(pair,n.selected,replace=FALSE)
selected <- c(selected,unlist(blocks[selected.blocks],use.names=FALSE))
}
unselected.blocks <- setdiff(pair,selected.blocks)
if(length(unselected.blocks)>0L) remaining.units <- c(remaining.units,blocks[unselected.blocks])
}

pooled.block <- which(block.size > 2L)
if(length(pooled.block)>0L){
pooled.block <- sample(pooled.block,length(pooled.block),replace=FALSE)
pooled <- unlist(blocks[pooled.block],use.names=FALSE)
ngroup <- ceiling(length(pooled)/group.size)
group.index <- rep(seq_len(ngroup),each=group.size,length.out=length(pooled))
n.selected <- floor(ngroup/2)
if(ngroup%%2L==1L&&runif(1)<0.5) n.selected <- n.selected+1L
selected.groups <- integer(0)
if(n.selected>0L){
selected.groups <- sample.int(ngroup,n.selected,replace=FALSE)
selected <- c(selected,pooled[group.index%in%selected.groups])
}
unselected.groups <- setdiff(seq_len(ngroup),selected.groups)
if(length(unselected.groups)>0L){
pooled.groups <- split(pooled,group.index)
remaining.units <- c(remaining.units,pooled.groups[unselected.groups])
}
}

selected <- sort(unique(selected))
target.min <- min(as.integer(min.size[1]), n)
if(length(selected)<target.min&&length(remaining.units)>0L){
remaining.units <- remaining.units[sample.int(length(remaining.units))]
for(unit in remaining.units){
selected <- c(selected,unit)
if(length(unique(selected))>=target.min) break
}
}
