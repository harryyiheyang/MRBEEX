...
CisMRBEEX=function(by,bX,byse,bXse,LD,Rxy,use.susie=T,
                   ...
                   tauvec=seq(3,30,by=3),rho=2,ridge.diff=10,
...
##########################################################################
if(use.susie==T){
A=Cis_MRBEE_IPOD_SuSiE(by=by,bX=bXest,byse=byse,bXse=bXestse,LD=LD,Rxy=Rxy,pip.thres=pip.thres,Lvec=Lvec,tauvec=tauvec,max.iter=max.iter,max.eps=max.eps,susie.iter=susie.iter,ebic.theta=ebic.theta,ebic.gamma=ebic.gamma,reliability.thres=reliability.thres,rho=rho,maxdiff=maxdiff,theta.ini=theta.ini,gamma.ini=gamma.ini,ridge=ridge.diff,pleiotropy.rm=pleiotropy.rm)
}
##########################################################################
if(use.susie==F){
A=Cis_MRBEE_IPOD(by=by,bX=bXest,byse=byse,bXse=bXestse,LD=LD,Rxy=Rxy,tauvec=tauvec,max.iter=max.iter,max.eps=max.eps,ebic.gamma=ebic.gamma,reliability.thres=reliability.thres,rho=rho,maxdiff=maxdiff,theta.ini=theta.ini,gamma.ini=gamma.ini,ebic.theta=ebic.theta,pleiotropy.rm=pleiotropy.rm)
}
A$bXest=bXest
A$bXestse=bXestse
A$bXest0=bXest0
