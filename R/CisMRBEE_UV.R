...
#' @param var.inf An indicator of whether using the formula of infinitesimal effect to estimate the standard error. Defaults to \code{T}.
...
#' @export
#'
CisMRBEE_UV=function(by,bX,byse,bXse,LD,Rxy,
                     xQTL.N,xQTL.max.L=10,xQTL.pip.thres=0.2,
                     reliability.thres=0.9,var.inf=T,
...
fit.susie=susie_rss(z=bX/bXse,R=LD,n=xQTL.N,L=sum(fit.susie$pip>xQTL.pip.thres)+1,max_iter=1000)
indj=which(fit.susie$pip>=xQTL.pip.thres)
if(length(indj)>0){
LDj=LD[indj,indj]
Thetaj=solve(LDj)
...
pleiotropy.rm=which(fit.susie$pip>xQTL.pip.thres)
...
if(sum(indgamma)>0){
Z=cbind(bXest0,diag(m)[,indgamma])
Hinv=matrixMultiply(t(Z),matrixMultiply(LD,Z))
Hinv[1,1]=Hinv[1,1]-sum(bXestse[indvalid])*Rxy[2,1]
...
h1=(xtx-sum(bXestse[indvalid])*Rxy[2,1])
covtheta=h0/h1/h1
}
