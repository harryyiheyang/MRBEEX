...
#' @param xQTLfit Initial SuSiE fit of the xQTL effects. Default is \code{NULL}.
...
H=matrixMultiply(Z,matrixMultiply(Theta,Z),transA=TRUE)
H[1,1]=H[1,1]-sum(bXestse[indvalid]^2)*Rxy[1,1]
Dmcp=c(0,ifelse(abs(gamma[indgamma])<3*tauvec[star],-1/3,0))
...
h1=(xtx-sum(bXestse[indvalid]^2)*Rxy[1,1])
covtheta=h0/h1/h1
}
