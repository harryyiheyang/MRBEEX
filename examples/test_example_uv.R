...
n.sim=1
Btheta=Bse=Bcov=matrix(0,n.sim,4)
while(iter<=n.sim){
...
t1=Sys.time()
fit.ipod=MRBEEX_UV(by=by,bX=bX,byse=byse,bXse=bXse,LD=LD,Rxy=Rxy,cluster.index=cluster.index,sampling.time=2,sampling.iter=2)
t2=Sys.time()
ipod.time=difftime(t2, t1, units = "secs")

t1=Sys.time()
fit.greedy=MRBEEX_UV(by=by,bX=bX,byse=byse,bXse=bXse,LD=LD,Rxy=Rxy,cluster.index=cluster.index,Method="Greedy",sampling.time=2,sampling.iter=2)
t2=Sys.time()
...
Btheta[iter,]=cbind(fit.ipod$theta,fit.greedy$theta,fit.ipod1$theta,fit.greedy1$theta)
