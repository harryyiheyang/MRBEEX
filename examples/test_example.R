...
t1=Sys.time()
fit.ipod=MRBEEX(method="IPOD",use.susie=F,by=by,bX=bX,byse=byse,bXse=bXse,LD=LD,Rxy=Rnn*Ruv,cluster.index=cluster.index,reliability.thres=0.8,tauvec=c(2.5,3,3.5,4:10,seq(12,30,2)),admm.rho=2,sampling.time=10,maxdiff=3,sampling.iter=5)
t2=Sys.time()
ipod.time=difftime(t2, t1, units = "secs")

t1=Sys.time()
fit.susie=MRBEEX(method="IPOD",use.susie=T,by,bX,byse,bXse,LD=LD,Lvec=c(1:6),pip.thres=0.2,Rxy=Ruv*Rnn,admm.rho=2,tauvec=c(2.5,3,3.5,4:10,seq(12,30,2)),ebic.gamma=1,cluster.index=cluster.index,sampling.time=10,maxdiff=3,reliability.thres=0.5,theta.ini=fit.ipod$theta,gamma.ini=fit.ipod$gamma,sampling.iter=5)
t2=Sys.time()
susie.time=difftime(t2, t1, units = "secs")

t1=Sys.time()
fit.mixture=MRBEEX(method="Mixture",use.susie=F,by,bX,byse,bXse,LD=LD,Rxy=Ruv*Rnn,ebic.gamma=1,cluster.index=cluster.index,sampling.time=10,maxdiff=3,reliability.thres=0.8,pip.thres=0.15,sampling.iter=5)
t2=Sys.time()
...
t1=Sys.time()
fit.mixture.susie=MRBEEX(method="Mixture",use.susie=T,by,bX,byse,bXse,LD=LD,Rxy=Ruv*Rnn,ebic.gamma=1,cluster.index=cluster.index,sampling.time=10,maxdiff=3,reliability.thres=0.8,sampling.iter=5)
t2=Sys.time()
