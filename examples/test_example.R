...
ridge.diff=1e5;verbose=T;pip.min=0.1;cred.pip.thres=0.95;group.penalize=F;group.index=c(1:ncol(bX)[1]);group.diff=10;coverage.causal=0.95;LDSC=NULL;Omega=NULL;estimate_residual_variance=T;estimate_residual_method="MoM"

t1=Sys.time()
fit.ipod=MRBEE_LDA(use.susie=F,by=by,bX=bX,byse=byse,bXse=bXse,LD=LD,Rxy=Rnn*Ruv,cluster.index=cluster.index,reliability.thres=0.8,tauvec=c(2.5,3,3.5,4,5,6),admm.rho=1,maxdiff=3)
t2=Sys.time()
ipod.time=difftime(t2, t1, units = "secs")

t1=Sys.time()
fit.susie=MRBEE_LDA(use.susie=T,by,bX,byse,bXse,LD=LD,Lvec=c(1:3),pip.thres=0.2,Rxy=Ruv*Rnn,admm.rho=2,tauvec=c(2.5,3,3.5,4:6),ebic.gamma=1,cluster.index=cluster.index,sampling.time=1,maxdiff=3,reliability.thres=0.5,theta.ini=fit.ipod$theta,gamma.ini=fit.ipod$gamma,sampling.iter=1,coverage.causal=0.9)
t2=Sys.time()
susie.time=difftime(t2, t1, units = "secs")

t1=Sys.time()
fit.susie1=MRBEE_LDA(use.susie=T,by,bX,byse,bXse,LD=LD,Lvec=c(1:3),pip.thres=0.2,Rxy=Ruv*Rnn,admm.rho=2,tauvec=c(2.5,3,3.5,4:6),ebic.gamma=1,cluster.index=cluster.index,sampling.time=1,maxdiff=3,reliability.thres=0.5,theta.ini=fit.ipod$theta,gamma.ini=fit.ipod$gamma,sampling.iter=1,coverage.causal=0.9)
t2=Sys.time()
susie.time=difftime(t2, t1, units = "secs")
